...
TEST_F(PostgresStatementTest, SqlIngestReplaceRejectsZeroColumnsBeforeDrop) {
  ASSERT_THAT(AdbcStatementNew(&connection, &statement, &error), IsOkStatus(&error));
  ASSERT_THAT(
      AdbcStatementSetSqlQuery(
          &statement, "CREATE TEMP TABLE adbc_zero_columns_replace_guard (id BIGINT)",
          &error),
      IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, nullptr, nullptr, &error),
              IsOkStatus(&error));
  ASSERT_THAT(
      AdbcStatementSetSqlQuery(
          &statement, "INSERT INTO adbc_zero_columns_replace_guard VALUES (42)", &error),
      IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, nullptr, nullptr, &error),
              IsOkStatus(&error));
  ...
  ASSERT_THAT(ArrowSchemaSetTypeStruct(schema.get(), 0), adbc_validation::IsOkErrno());
  nanoarrow::UniqueArrayStream bind;
  nanoarrow::EmptyArrayStream(schema.get()).ToArrayStream(bind.get());

  ASSERT_THAT(AdbcStatementSetOption(&statement, ADBC_INGEST_OPTION_TARGET_TABLE,
                                     "adbc_zero_columns_replace_guard", &error),
              IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementSetOption(&statement, ADBC_INGEST_OPTION_MODE,
                                     ADBC_INGEST_OPTION_MODE_REPLACE, &error),
              IsOkStatus(&error));
  ...
  // Reject the schema before DROP, preserving both the target and the statement.
  ASSERT_THAT(
      AdbcStatementSetSqlQuery(
          &statement, "SELECT id FROM pg_temp.adbc_zero_columns_replace_guard", &error),
      IsOkStatus(&error));
  adbc_validation::StreamReader reader;
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, &reader.stream.value,
                                        &reader.rows_affected, &error),
...
TEST_F(PostgresStatementTest, SqlIngestReplaceRejectsUnsupportedTypeBeforeDrop) {
  ...
  ASSERT_THAT(AdbcStatementSetSqlQuery(
                  &statement,
                  "CREATE TEMP TABLE adbc_unsupported_replace_guard (id BIGINT)", &error),
              IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, nullptr, nullptr, &error),
              IsOkStatus(&error));
  ASSERT_THAT(
      AdbcStatementSetSqlQuery(
          &statement, "INSERT INTO adbc_unsupported_replace_guard VALUES (42)", &error),
      IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, nullptr, nullptr, &error),
              IsOkStatus(&error));
  ...
  ArrowSchemaInit(schema.get());
  ASSERT_THAT(ArrowSchemaSetTypeStruct(schema.get(), 1), adbc_validation::IsOkErrno());
  ASSERT_THAT(ArrowSchemaSetTypeStruct(schema->children[0], 0),
              adbc_validation::IsOkErrno());
  ASSERT_THAT(ArrowSchemaSetName(schema->children[0], "payload"),
              adbc_validation::IsOkErrno());
  nanoarrow::UniqueArrayStream bind;
  nanoarrow::EmptyArrayStream(schema.get()).ToArrayStream(bind.get());

  ASSERT_THAT(AdbcStatementSetOption(&statement, ADBC_INGEST_OPTION_TARGET_TABLE,
                                     "adbc_unsupported_replace_guard", &error),
              IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementSetOption(&statement, ADBC_INGEST_OPTION_MODE,
                                     ADBC_INGEST_OPTION_MODE_REPLACE, &error),
              IsOkStatus(&error));
  ...
  // Type resolution fails before replacement DDL, and the statement recovers.
  ASSERT_THAT(
      AdbcStatementSetSqlQuery(
          &statement, "SELECT id FROM pg_temp.adbc_unsupported_replace_guard", &error),
      IsOkStatus(&error));
  adbc_validation::StreamReader reader;
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, &reader.stream.value,
                                        &reader.rows_affected, &error),
...
struct CopyFailingStreamState {
  ArrowArrayStream* source;
  int next_calls = 0;
  PGconn* killer = nullptr;
  int backend_pid = 0;
  bool terminated = false;
};

static ArrowArrayStream MakeCopyFailingStream(CopyFailingStreamState* state) {
  ArrowArrayStream stream = {};
  stream.get_schema = [](ArrowArrayStream* stream, ArrowSchema* out) {
    auto* state = static_cast<CopyFailingStreamState*>(stream->private_data);
    return state->source->get_schema(state->source, out);
  };
  stream.get_next = [](ArrowArrayStream* stream, ArrowArray* out) {
    auto* state = static_cast<CopyFailingStreamState*>(stream->private_data);
    if (state->next_calls++ == 0) {
      return state->source->get_next(state->source, out);
    }
    // The first batch has been serialized and sent before COPY asks for more.
    if (state->killer != nullptr) {
      const std::string query =
          "SELECT pg_terminate_backend(" + std::to_string(state->backend_pid) + ")";
      adbc::driver::pgwire::UniqueResult terminate(PQexec(state->killer, query.c_str()));
      state->terminated = terminate != nullptr &&
                          PQresultStatus(terminate.get()) == PGRES_TUPLES_OK &&
                          PQntuples(terminate.get()) == 1 &&
                          std::strcmp(PQgetvalue(terminate.get(), 0, 0), "t") == 0;
    }
    out->release = nullptr;
    return EIO;
  };
  stream.get_last_error = [](ArrowArrayStream*) {
    return "injected COPY stream read failure";
  };
  stream.release = [](ArrowArrayStream* stream) {
...
TEST_F(PostgresStatementTest, SqlIngestCopyStreamErrorAfterFirstBatchPreservesTarget) {
  auto& connection_impl = *reinterpret_cast<std::shared_ptr<adbcpq::PostgresConnection>*>(
      connection.private_data);
  PGconn* pg_conn = connection_impl->conn();
  ASSERT_THAT(AdbcStatementNew(&connection, &statement, &error), IsOkStatus(&error));
  ASSERT_THAT(
      AdbcStatementSetSqlQuery(
          &statement, "CREATE TEMP TABLE adbc_copy_stream_error (id BIGINT)", &error),
      IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, nullptr, nullptr, &error),
              IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementSetSqlQuery(
                  &statement, "INSERT INTO adbc_copy_stream_error VALUES (42)", &error),
              IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, nullptr, nullptr, &error),
              IsOkStatus(&error));
  ...
  ASSERT_THAT(
      adbc_validation::MakeBatch<int64_t>(schema.get(), batch.get(), nullptr, {1, 2, 3}),
      adbc_validation::IsOkErrno());
  ...
  // COPY must be aborted, not successfully finished with the preceding batch.
  // This also proves its terminal result was drained before the next query.
  ASSERT_THAT(AdbcStatementSetSqlQuery(
                  &statement, "SELECT id FROM pg_temp.adbc_copy_stream_error", &error),
              IsOkStatus(&error));
  {
    adbc_validation::StreamReader reader;
    ASSERT_THAT(AdbcStatementExecuteQuery(&statement, &reader.stream.value,
  ...
  // The same statement can subsequently execute another successful COPY.
  nanoarrow::UniqueSchema retry_schema;
  ArrowSchemaInit(retry_schema.get());
  ASSERT_THAT(ArrowSchemaSetTypeStruct(retry_schema.get(), 1),
              adbc_validation::IsOkErrno());
  ASSERT_THAT(ArrowSchemaSetType(retry_schema->children[0], NANOARROW_TYPE_INT64),
              adbc_validation::IsOkErrno());
  ASSERT_THAT(ArrowSchemaSetName(retry_schema->children[0], "id"),
              adbc_validation::IsOkErrno());
  nanoarrow::UniqueArray retry_batch;
  ASSERT_THAT(adbc_validation::MakeBatch<int64_t>(retry_schema.get(), retry_batch.get(),
                                                  nullptr, {5, 6}),
              adbc_validation::IsOkErrno());
  ASSERT_THAT(AdbcStatementSetOption(&statement, ADBC_INGEST_OPTION_TARGET_TABLE,
                                     "adbc_copy_stream_error", &error),
...
TEST_F(PostgresStatementTest, SqlIngestCopyServerErrorPreservesSqlStateAndTarget) {
  auto& connection_impl = *reinterpret_cast<std::shared_ptr<adbcpq::PostgresConnection>*>(
      connection.private_data);
  PGconn* pg_conn = connection_impl->conn();
  ASSERT_THAT(AdbcStatementNew(&connection, &statement, &error), IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementSetSqlQuery(&statement,
                                       "CREATE TEMP TABLE adbc_copy_server_error "
  ...
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, nullptr, nullptr, &error),
              IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementSetSqlQuery(
                  &statement, "INSERT INTO adbc_copy_server_error VALUES (42)", &error),
              IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, nullptr, nullptr, &error),
              IsOkStatus(&error));
  ...
  ASSERT_THAT(adbc_validation::MakeBatch<int64_t>(schema.get(), first_batch.get(),
                                                  nullptr, {1, 2, 3}),
              adbc_validation::IsOkErrno());
  ...
  int64_t rows_affected = -1;
  EXPECT_THAT(AdbcStatementExecuteQuery(&statement, nullptr, &rows_affected, &error),
              IsStatus(ADBC_STATUS_INTEGRITY, &error));
  EXPECT_EQ(std::string(error.sqlstate, 5), "23514");
  EXPECT_EQ(rows_affected, 0);
  ASSERT_NE(error.release, nullptr);
  error.release(&error);
  error = ADBC_ERROR_INIT;
  ...
  ASSERT_THAT(AdbcStatementSetSqlQuery(
                  &statement, "SELECT id FROM pg_temp.adbc_copy_server_error", &error),
              IsOkStatus(&error));
  adbc_validation::StreamReader reader;
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, &reader.stream.value,
                                        &reader.rows_affected, &error),
...
TEST_F(PostgresStatementTest, SqlIngestCopyStreamErrorLeavesCallerTransactionAborted) {
  auto& connection_impl = *reinterpret_cast<std::shared_ptr<adbcpq::PostgresConnection>*>(
      connection.private_data);
  PGconn* pg_conn = connection_impl->conn();
  ASSERT_THAT(AdbcStatementNew(&connection, &statement, &error), IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementSetSqlQuery(
                  &statement,
                  "CREATE TEMP TABLE adbc_copy_caller_transaction (id BIGINT)", &error),
              IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, nullptr, nullptr, &error),
              IsOkStatus(&error));
  ASSERT_THAT(
      AdbcStatementSetSqlQuery(
          &statement, "INSERT INTO adbc_copy_caller_transaction VALUES (42)", &error),
      IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, nullptr, nullptr, &error),
              IsOkStatus(&error));
  ASSERT_THAT(AdbcConnectionSetOption(&connection, ADBC_CONNECTION_OPTION_AUTOCOMMIT,
  ...
  ArrowArrayStream failing_stream = MakeCopyFailingStream(&state);
  ASSERT_THAT(AdbcStatementSetOption(&statement, ADBC_INGEST_OPTION_TARGET_TABLE,
                                     "adbc_copy_caller_transaction", &error),
              IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementSetOption(&statement, ADBC_INGEST_OPTION_MODE,
                                     ADBC_INGEST_OPTION_MODE_APPEND, &error),
              IsOkStatus(&error));
  ...
  error.release(&error);
  error = ADBC_ERROR_INIT;
  // The driver must drain COPY without rolling back a transaction it does not own.
  ...
  ASSERT_THAT(
      AdbcStatementSetSqlQuery(
          &statement, "SELECT id FROM pg_temp.adbc_copy_caller_transaction", &error),
      IsOkStatus(&error));
  adbc_validation::StreamReader reader;
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, &reader.stream.value,
                                        &reader.rows_affected, &error),
...
TEST_F(PostgresStatementTest, SqlIngestCopyAbortFailureMakesConnectionUnusable) {
  auto& connection_impl = *reinterpret_cast<std::shared_ptr<adbcpq::PostgresConnection>*>(
      connection.private_data);
  PGconn* pg_conn = connection_impl->conn();
  adbc::driver::pgwire::UniqueConnection killer(
      PQconnectdb(std::getenv("ADBC_POSTGRESQL_TEST_URI")));
  ASSERT_NE(killer, nullptr);
  ASSERT_EQ(PQstatus(killer.get()), CONNECTION_OK) << PQerrorMessage(killer.get());
  ASSERT_THAT(AdbcStatementNew(&connection, &statement, &error), IsOkStatus(&error));
  ASSERT_THAT(
      AdbcStatementSetSqlQuery(
          &statement, "CREATE TEMP TABLE adbc_copy_abort_failure (id BIGINT)", &error),
      IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, nullptr, nullptr, &error),
              IsOkStatus(&error));
  ...
  nanoarrow::UniqueArrayStream source;
  adbc_validation::MakeStream(source.get(), schema.get(), std::move(batches));
  CopyFailingStreamState state{source.get(), 0, killer.get(), PQbackendPID(pg_conn)};
  ArrowArrayStream failing_stream = MakeCopyFailingStream(&state);
  ASSERT_THAT(AdbcStatementSetOption(&statement, ADBC_INGEST_OPTION_TARGET_TABLE,
                                     "adbc_copy_abort_failure", &error),
              IsOkStatus(&error));
  ASSERT_THAT(AdbcStatementSetOption(&statement, ADBC_INGEST_OPTION_MODE,
                                     ADBC_INGEST_OPTION_MODE_APPEND, &error),
              IsOkStatus(&error));
  ...
  EXPECT_EQ(rows_affected, 0);
  ASSERT_NE(error.message, nullptr);
  // Preserve the original Arrow failure even when COPY termination also fails.
  EXPECT_THAT(error.message, ::testing::HasSubstr("injected COPY stream read failure"));
  ASSERT_NE(error.release, nullptr);
  error.release(&error);
  error = ADBC_ERROR_INIT;
  ...
  ASSERT_THAT(AdbcStatementExecuteQuery(&statement, nullptr, nullptr, &error),
              IsStatus(ADBC_STATUS_INVALID_STATE, &error));
  ASSERT_NE(error.message, nullptr);
  EXPECT_THAT(error.message, ::testing::HasSubstr("COPY ingest cleanup failed"));
  error.release(&error);
  error = ADBC_ERROR_INIT;
  ASSERT_THAT(AdbcStatementRelease(&statement, &error), IsOkStatus(&error));
  ASSERT_THAT(AdbcConnectionRelease(&connection, &error), IsOkStatus(&error));
...
TEST_F(PostgresStatementTest, BindTimezoneSetupErrorRollsBackOwnedTransaction) {
  auto& connection_impl = *reinterpret_cast<std::shared_ptr<adbcpq::PostgresConnection>*>(
      connection.private_data);
  PGconn* pg_conn = connection_impl->conn();
  ASSERT_EQ(PQtransactionStatus(pg_conn), PQTRANS_IDLE);
  ...
  const std::string schema =
      "adbc_tz_setup_fault_" + std::to_string(PQbackendPID(pg_conn));
  adbc::driver::pgwire::UniqueResult setup(
      PQexec(pg_conn, ("CREATE SCHEMA " + schema + "; CREATE FUNCTION " + schema +
                       ".current_setting(text) RETURNS text LANGUAGE plpgsql AS $$ BEGIN "
                       "RAISE EXCEPTION 'injected timezone lookup failure'; RETURN NULL; "
                       "END $$; SET TIME ZONE 'Europe/Berlin'; SET search_path TO " +
                       schema + ", pg_catalog")
                          .c_str()));
  ASSERT_EQ(PQresultStatus(setup.get()), PGRES_COMMAND_OK);
  struct FaultCleanup {
    ...
    ~FaultCleanup() {
      if (PQtransactionStatus(conn) != PQTRANS_IDLE) {
        adbc::driver::pgwire::UniqueResult rollback(PQexec(conn, "ROLLBACK"));
      }
      adbc::driver::pgwire::UniqueResult reset(PQexec(conn, "RESET search_path"));
      adbc::driver::pgwire::UniqueResult drop(
          PQexec(conn, ("DROP SCHEMA IF EXISTS " + schema + " CASCADE").c_str()));
    }
  } cleanup{pg_conn, schema};
  ...
  for (bool request_output : {false, true}) {
    ...
    nanoarrow::UniqueArrayStream bind;
    nanoarrow::EmptyArrayStream(bind_schema.get()).ToArrayStream(bind.get());
    ASSERT_THAT(AdbcStatementBindStream(&bound_statement.value, bind.get(), &error),
                IsOkStatus(&error));
    ...
    ASSERT_THAT(AdbcStatementExecuteQuery(&bound_statement.value,
                ...
                IsStatus(ADBC_STATUS_IO, &error));
    ASSERT_NE(error.message, nullptr);
    EXPECT_THAT(error.message, ::testing::HasSubstr("injected timezone lookup failure"));
    error.release(&error);
    error = ADBC_ERROR_INIT;
...
static int MakeTimezoneAndDivisorBind(ArrowSchema* schema, ArrowArray* batch) {
  ArrowSchemaInit(schema);
  NANOARROW_RETURN_NOT_OK(ArrowSchemaSetTypeStruct(schema, 2));
  NANOARROW_RETURN_NOT_OK(ArrowSchemaSetTypeDateTime(
      schema->children[0], NANOARROW_TYPE_TIMESTAMP, NANOARROW_TIME_UNIT_MICRO, "UTC"));
  NANOARROW_RETURN_NOT_OK(ArrowSchemaSetType(schema->children[1], NANOARROW_TYPE_INT32));
  const std::vector<std::optional<int64_t>> timestamps{0, 0};
  const std::vector<std::optional<int32_t>> divisors{2, 0};
  return adbc_validation::MakeBatch<int64_t, int32_t>(schema, batch, nullptr, timestamps,
                                                      divisors);
}

TEST_F(PostgresStatementTest, BindRowErrorRollsBackOwnedTimezoneTransaction) {
  auto& connection_impl = *reinterpret_cast<std::shared_ptr<adbcpq::PostgresConnection>*>(
      connection.private_data);
  PGconn* pg_conn = connection_impl->conn();
  ASSERT_EQ(PQtransactionStatus(pg_conn), PQTRANS_IDLE);
  adbc::driver::pgwire::UniqueResult setup(PQexec(
      ...
      "CREATE TEMP TABLE adbc_tz_bind_row_failure (ts TIMESTAMPTZ, quotient INTEGER)"));
  ASSERT_EQ(PQresultStatus(setup.get()), PGRES_COMMAND_OK);
  ...
  for (bool request_output : {false, true}) {
    ...
    ASSERT_THAT(
        AdbcStatementSetSqlQuery(&bound_statement.value,
                                 "INSERT INTO pg_temp.adbc_tz_bind_row_failure VALUES "
    ...
    if (request_output) {
      ...
      ASSERT_NO_FATAL_FAILURE(reader.Next());
      ASSERT_EQ(reader.array->length, 1);
      EXPECT_EQ(ArrowArrayViewGetIntUnsafe(reader.array_view->children[0], 0), 5);
      ...
      // A failed stream must not resume the remaining bind outside its transaction.
      EXPECT_EQ(reader.MaybeNext(), 0);
      EXPECT_EQ(reader.array->release, nullptr);
    } else {
      ASSERT_THAT(AdbcStatementExecuteQuery(&bound_statement.value, nullptr,
                                            &rows_affected, &error),
                  IsStatus(ADBC_STATUS_INVALID_DATA, &error));
      EXPECT_EQ(rows_affected, 0);
      ASSERT_NE(error.message, nullptr);
      EXPECT_THAT(error.message, ::testing::HasSubstr("division by zero"));
      error.release(&error);
      error = ADBC_ERROR_INIT;
    }
    ...
    adbc::driver::pgwire::UniqueResult count(
        PQexec(pg_conn, "SELECT COUNT(*) FROM pg_temp.adbc_tz_bind_row_failure"));
    ASSERT_EQ(PQresultStatus(count.get()), PGRES_TUPLES_OK);
    EXPECT_STREQ(PQgetvalue(count.get(), 0, 0), "0");
  }
}

TEST_F(PostgresStatementTest, BindRowErrorLeavesExplicitTransactionToCaller) {
  auto& connection_impl = *reinterpret_cast<std::shared_ptr<adbcpq::PostgresConnection>*>(
      connection.private_data);
  PGconn* pg_conn = connection_impl->conn();
  adbc::driver::pgwire::UniqueResult setup(
      PQexec(pg_conn,
             "SET TIME ZONE 'Europe/Berlin'; "
             "CREATE TEMP TABLE adbc_tz_bind_explicit_failure (ts TIMESTAMPTZ, quotient "
             "INTEGER)"));
  ASSERT_EQ(PQresultStatus(setup.get()), PGRES_COMMAND_OK);
  ASSERT_THAT(AdbcConnectionSetOption(&connection, ADBC_CONNECTION_OPTION_AUTOCOMMIT,
                                      ADBC_OPTION_VALUE_DISABLED, &error),
  ...
  ASSERT_THAT(
      AdbcStatementSetSqlQuery(&bound_statement.value,
                               "INSERT INTO pg_temp.adbc_tz_bind_explicit_failure VALUES "
                               "($1::timestamptz, 10 / $2::integer) RETURNING quotient",
                               &error),
      IsOkStatus(&error));
  nanoarrow::UniqueSchema bind_schema;
  ...
  EXPECT_STREQ(PQgetvalue(timezone.get(), 0, 0), "Europe/Berlin");
  adbc::driver::pgwire::UniqueResult count(
      PQexec(pg_conn, "SELECT COUNT(*) FROM pg_temp.adbc_tz_bind_explicit_failure"));
  ASSERT_EQ(PQresultStatus(count.get()), PGRES_TUPLES_OK);
  EXPECT_STREQ(PQgetvalue(count.get(), 0, 0), "0");
}
