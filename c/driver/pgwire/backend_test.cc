...
TEST(BackendProfileTest, BulkIngestModeIsCapabilityDriven) {
  ...
  EXPECT_EQ(BackendProfile::PostgreSQL().capabilities.parameterized_ingest_batch_rows, 1);
  EXPECT_EQ(BackendProfile::Redshift().capabilities.parameterized_ingest_batch_rows, 16);
}
